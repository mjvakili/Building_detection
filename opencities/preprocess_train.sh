...
python -m preprocessor.resample --path_pattern=$INTER_DATA_DIR/train_tier_1/*/*/*.tif --dst_res=0.1

# for each resampled tif file create raster mask from provided geometries in .geojson file
echo "Creating raster masks"
python -m preprocessor.generate_masks --path_pattern=$INTER_DATA_DIR/train_tier_1/*/*/res-*.tif

# slicing big tif files to small ones with size 1024x1024
echo "Slicing tif files..."
python -m preprocessor.cut_train \
